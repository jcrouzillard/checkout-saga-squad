...
**Revalidação (2ª rodada)**: base `89e5834` (correção do Frontend, handoff `e2eb4f62c464`). Ver a seção "Revalidação" no fim.
...
| **T5** 1440 | **PASSA** (2ª rodada: índice marca Gates) | `#/demandas/D14/gates`: a mesma tela e o mesmo alvo depois de F5 e em outra aba (a D14 tinha decisão pendente, então a página rola até a Próxima ação com foco na nota, como previsto). D16 sem pendência: a seção Gates fica no topo. F5 também preserva `?agente=backend` (gaveta aberta), `?f=backlog` e `?demanda=D14&tipo=gate` na Auditoria. ~~Ressalva DEF-2~~: corrigido, `toc` = "Gates" na carga, após F5 e em outra aba. |
...
| CA-3 | PASSA | 28 rotas testadas a 1440 e a 390, inclusive `D13` e o id do log, `D999` ("Demanda não encontrada"), filtros e gaveta. Os aliases `#execucoes`, `#/decisoes`, `#/evidencias`, `#/politicas`, `#/agentes` e `#/observabilidade` redirecionam. `#/xyz` vai ao Painel com aviso. Voltar funciona: D13 → Demandas → Painel. F5 preserva a rota (T5). DEF-4 corrigido na 2ª rodada. |
...
| CA-6 | PASSA | Páginas conferidas: ativa (D13, D14), na fila (D17, D18), backlog (D22), entregue (D11), cancelada (D15, D19), devolvida pelo revisor (D21) e aprovada abrindo PR (D16). DEF-2 e DEF-5 corrigidos na 2ª rodada. |
| CA-7 | **PASSA** (2ª rodada) | A ordem 1 aparece (badge vermelho, "Decidir G2") e some depois da decisão. A ordem 3 (PR #104 da D12) e a ordem 4 (Iniciar) funcionam. **DEF-1**: a ordem 5 (validação atrasada) continua listando demandas **já iniciadas** (D17 vinda do backlog e D20 iniciada com override) e infla o badge. **Corrigido em `89e5834`**: D17 e D20 somem de "Precisa de você" depois de iniciadas; a D23 (atrasada e não iniciada, controle positivo) continua listada. |
...
| CA-15 | **PASSA** (2ª rodada) | ~~DEF-3~~ (corrigido): os links de PR de `reviewHtml` (Entregues recentemente PR #106/#95/#90; cabeçalho de entregue ou devolvida) não têm "↗" nem "(abre em nova aba)". O mesmo ocorre na issue do item de backlog (l. 892, visto no código). Os demais externos estão conformes, e os internos usam `#/…`. Na 2ª rodada: PR #106/#95/#90 (Entregues), PR #106 e issue #96 (D11), PR #901 (devolvida, D21) e issue #920 do backlog (fixture em `github-sync.json` da cópia) têm "↗" e "(abre em nova aba)"; a varredura de 28 rotas × 2 larguras não acha externo sem seta. |
...
## Defeitos (dono: Frontend, `squad-control/index.html`) — todos corrigidos em `89e5834` (ver Revalidação)
- ~~**DEF-1 (médio, CA-7)**~~ **CORRIGIDO**: `nextAction` ordem 5 (`vinfo.st === "atrasada"`) não exige `preStart`. Demandas já iniciadas sem
...
- ~~**DEF-2 (baixo, §7/CA-6)**~~ **CORRIGIDO**: `renderDemanda` lê a seção em `route.parts[2]` (l. 685), mas a rota `#/demandas/Dn/<secao>` a
...
- ~~**DEF-3 (baixo, CA-15)**~~ **CORRIGIDO**: os links de PR em `reviewHtml` (l. 826/828) e a issue do item de backlog (l. 892) não usam
  `extLink`, então ficam sem "↗" e sem "(abre em nova aba)".
- ~~**DEF-4 (baixo, CA-3)**~~ **CORRIGIDO**: depois de `#/xyz`, ir para `#/painel#squad-base` mantém o aviso "Página não encontrada" (l. 1154:
  o aviso só é limpo quando a rota já é canônica).
- ~~**DEF-5 (baixo, CA-6)**~~ **CORRIGIDO**: na seção Gates, cada ciclo de um mesmo gate mostra a **última** decisão humana (`humanFor` sem
  limite superior). O 1º G2 da D14, decidido às 12:33:25 (T2), aparece com a decisão das 12:39:34 (`d13-demanda-gates-1440.png`).
- **CORRIGIDO** (`ctlConfirm`): observação que já existia desde o D7, não é regressão do D13: "Cancelar demanda" (em andamento) guarda a confirmação no nó
...
## Revalidação — 2ª rodada (base `89e5834`)
Servidores do worktree: **7116** = cópia da memória real (docs/squad, AGENTS.md, CLAUDE.md, `.squad/runs`), **somente leitura**
(0 POSTs; o log real ficou com o mesmo md5 antes e depois). **7117** = cópia temporária com fixtures. O script agora é
**autossuficiente**: cria as fixtures base D14–D17 e o run do Backend (`FIX_PID`) quando faltam; D20/D21/D22 e a D23
(controle positivo do DEF-1) entram no próprio script. Cenário novo: `revalidacao-1440` (`ONLY=revalidacao` roda só ele).
Nenhum erro de console ou `pageerror`.

| Item | Resultado | Evidência |
|---|---|---|
| DEF-1 | **CORRIGIDO** | Antes do override, a D20 aparece como "Validação atrasada" (correto, ainda não iniciada). Depois de T6 (D17 movida do backlog) e do PA-3 override (D20), "Precisa de você" = só D12 (PR #104) e D23. A D23 (atrasada há 7 min e sem início) continua com "Validação atrasada". Badge do Painel = 1 na fixture. |
| DEF-2 | **CORRIGIDO** | `#/demandas/D16/{execucao,gates,validacao,registro}`: `aria-current="location"` em Execução, Gates, Validação e Registro. Em `registro`, o `details` do Registro abre. `#/demandas/D16` sem seção não marca nada. |
| DEF-3 | **CORRIGIDO** | Ver CA-15. |
| DEF-4 | **CORRIGIDO** | `#/xyz` → Painel com aviso. Em seguida, `#/painel#squad-base` e `#/demandas` ficam sem aviso. Um novo `#/abc` volta a mostrar o aviso. |
| DEF-5 | **CORRIGIDO** | D14, seção Gates: o 1º G2 (RETURN 62%) mostra "APPROVE · QA T2 toast…" e o 2º G2 (RETURN 60%) mostra "APPROVE · QA T10 teclado…", cada decisão no seu ciclo (`d13-demanda-gates-1440.png`). |
| Cancelar no polling | **CORRIGIDO** | D14: "Cancelar demanda" → "Confirmar cancelamento". Com um evento novo no log e 3 polls em 8 s, o botão continua "Confirmar cancelamento" (`data-confirm=1`). Navegar para a D16 e voltar à D14 limpa a confirmação. 0 POSTs. PA-5, PA-7 e PA-7-cancelar (2 cliques) seguem funcionando. |
| T1–T10 | **PASSA** | Todos os `ok` = true: T1 (3 cliques depois de registrar, sem menu), T2 toast (2), T2 sino (3), T3 (1), T4 (2), T5, T6 (4), T7 (3), T8 (2), T9 (6+ polls) e T10 (Shift+Tab ×10 até o skip, Tab ×3 até "Decidir G2"). |
| X11 (T2) | **PASSA** | As decisões vão ao gate exibido: `human` G2/`fx14aaaaaaaa` APPROVE (toast e teclado) e G2/`fx15bbbbbbbb` OVERRIDE (sino), enquanto a D16 (a última ativa) recebia G1, G2 e G3 depois. O gate da D16 não foi afetado. |
| Paridade e CA | **PASSA** | PA-1..PA-23 como na 1ª rodada (PA-15: 407 eventos, 341 selos). Rotas 28 × 2 larguras: um item de menu corrente, título certo, 1 `h1`, sem rolagem horizontal, faixa oculta só em Produto, aviso só em `#/xyz`, Voltar D13 → Demandas → Painel. Requisições só `GET /api/state|project|policy`. PA-7 na D16 (G3 aprovado): o servidor ainda recusa o cancelamento (409, regra anterior e não regressão). |

Capturas regeneradas nesta rodada: todas as `tests/ui/d13-*.png` listadas acima.
