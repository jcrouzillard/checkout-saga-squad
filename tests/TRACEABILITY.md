...
| Consultar status do pedido | `GET /orders/{orderId}` com `history` (`api.md` §1) | e2e: os 7 cenários fazem poll de `GET /orders/{orderId}` via `wait_for_status` (`tests/e2e/run.sh`) | ✅ passou |
