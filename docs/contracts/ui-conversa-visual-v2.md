...
| d | Horários em **fusos diferentes** (08:47:59 no cabeçalho × 11:47:11 no texto) | A UI está certa: `hhmm()` usa `toLocaleTimeString` → fuso do navegador (BRT, UTC−3 → 08:47:59). O **11:47:11 é texto do modelo**: o contexto `<dados_da_squad>` montado por `build_context()` repassa `eventosRecentes[].ts` crus do `decisions.jsonl`, que estão em **UTC** (`2026-09-25T11:47:11+00:00`, gerado por `log.py` com `timezone.utc`), sem dizer o fuso do humano; o prompt `docs/squad/prompts/conversa.md` não tem regra de horário; e o modelo também lê o `decisions.jsonl` direto (ferramenta Read), igualmente em UTC. **A causa é do servidor/prompt**, não da UI (correção exata no §3.4). Agravante de UI: horário com segundos em fonte mono, sem data, dificulta comparar. | `tools/squad/conversa.py` `build_context()` (def l. ~223; `ts` cru l. ~249, `now_iso()` l. ~257); `docs/squad/prompts/conversa.md` |
...
2. **Caminho do `tz`** (sem estado global): `server.py` (rota `mensagens`, l. ~1233) passa
   `eng.send(cid, data.get("text"), t0=…, tz=data.get("tz"))` → `Engine.send()` (`conversa.py` l. ~623) valida
   (regex + `ZoneInfo`, fallback `None` = fuso do servidor) e guarda no `Turn` (ou repassa em
   `threading.Thread(target=self._run, args=(t, runner, text))`) → `Engine._run()` chama
   `build_context(self.state_fn(), self.store.data_root, tz=t.tz)` (l. ~694). Assinatura nova:
   `build_context(state, data_root=None, tz: str | None = None)` — parâmetro opcional; chamadas existentes inalteradas.
3. `build_context()` recebe o fuso e: (i) converte `eventosRecentes[].ts` para ISO **com o deslocamento local**
   ...
   nem `log.py`).
4. `docs/squad/prompts/conversa.md` ganha a regra: "Horários: cite sempre no fuso indicado em `<dados_da_squad fuso>`,
...
| CA-V5 (d) — servidor | Contexto e prompt no fuso do humano | Teste unitário: `build_context(state, data_root, tz="America/Sao_Paulo")` com evento `ts` `2026-09-25T11:47:11+00:00` → `eventosRecentes[0].ts == "2026-09-25T08:47:11-03:00"` e cabeçalho com `fuso="America/Sao_Paulo" utc="-03:00"`; `tz` inválido (`"../x"`, 65 chars) → fuso do servidor, resposta `202` inalterada. `conversa.md` contém a regra do §3.4.3. Registros `.jsonl` e `decisions.jsonl` continuam com `Z`/`+00:00`. Com runner real: pergunta "a que horas registrei as respostas da D19?" → horário citado igual ao exibido no painel de demandas (fuso local). |
...
| CA-V12 | Sem regressão | CA-1…CA-23 do contrato D17 que tocam a UI (CA-1, 2, 3, 10, 18, 19, 21, 23) continuam passando; `tests/squad/*.py` passam; o indicador `.live` do cabeçalho do Squad Control mantém a aparência de antes. Verificação feita **após o rebase sobre a D18 (PR #171)** na `develop` (D18 altera `#hdr .live`/`.live .lw`; D20 não toca a regra global `.live`). |
