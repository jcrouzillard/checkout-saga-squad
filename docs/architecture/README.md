...
| **Escalabilidade** | Serviços stateless; 3 partições permitem até 3 consumidores por grupo; scheduler em duas fases (`SKIP LOCKED` + lock otimista) suporta N réplicas do orquestrador (cada uma com seu `KAFKA_GROUP_INSTANCE_ID`; ADR-013) | **HPA por lag de consumo** (KEDA) e CPU; mais partições conforme volume; particionar/arquivar tabelas `outbox`/`processed_messages` com limpeza por TTL |
